...
function FilterPlaylist() {
  ...
  const shouldRenderPlaylist = (playlist: Playlist): boolean => {
    const canEdit = playlist.collaborative || (currentUser !== undefined && playlist.owner.id == currentUser.id)

    return canEdit && playlist.images.length >= 1 && playlist.tracks.total > 0
  }
  ...
  return (
    <>
      <div className="md:ml-20 md:mr-[68px] mx-3 mb-4 transition-all">
        <div className={modalOpen ? "blur-sm" : "blur-none"}>
          {currentUser !== undefined && (
            <div className="my-4 mx-1 text-2xl font-bold">{currentUser.display_name}'s Playlists</div>
          )}
