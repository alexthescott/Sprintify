...
function getPlaylists(): ApiCall<Playlist[]> {
  return new ApiCall<Playlist[]>(
    ...
    (response: any | any[]) => {
      if (Array.isArray(response)) return response.flatMap((res: any) => res.items as Playlist[])
...
function getTrackFeatures(tracks: string[]): ApiCall<TrackFeatures[]>[] {
  ...
  for (let i = 0; i < tracks.length; i += 100) {
    // Api only supports 100 tracks at a time
    apiCallArgs.push({
  ...
  return apiCallArgs.map((args) => new ApiCall<TrackFeatures[]>(args, (values: any[]) => values[0].audio_features))
}
...
async function populateTrackFeatures(tracks: Track[]): Promise<Track[]> {
  const features = (await Promise.all(getTrackFeatures(tracks.map((track) => track.id)).map((t) => t.result))).flatMap(
    (f) => f
  )
